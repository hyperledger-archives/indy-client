...
sendNymHelpMsg = HelpMsg("send NYM", "Adds given identifier to sovrin",
...
sendGetNymHelpMsg = HelpMsg("send GET_NYM", "Get NYM from sovrin",
                            "send GET_NYM dest=<target identifier>",
                            ["send GET_NYM dest=33A18XMqWqTzDpLHXLR5nT"])

sendAttribHelpMsg = HelpMsg("send ATTRIB", "Adds attributes to existing identifier",
...
sendNodeHelpMsg = HelpMsg("send NODE", "Adds a node to the pool",
...
sendSchemaMsg = HelpMsg("send SCHEMA", "Adds schema to sovrin",
...
sendIssuerHelpMsg = HelpMsg("send ISSUER_KEY", "Adds issuer keys for given schema",
                            "send ISSUER_KEY ref=<ref-no-of-SCHEMA-txn>",
                            ["send ISSUER_KEY ref=10"])

showFileHelpMsg = HelpMsg("show file", "Shows content of given file",
                          "show <file-path>",
                          ["show sample/faber-invitation.sovrin"])

loadFileHelpMsg = HelpMsg("load file", "Creates the link",
                          "load <file-path>",
                          ["load sample/faber-invitation.sovrin"])

showLinkHelpMsg = HelpMsg("show link", "Shows link info in case of one matching link, otherwise shows all the matching link names",
                          "show link <link-name>", ["show link faber"])

connectToHelpMsg = HelpMsg("connect", "Lets you connect to the respective environment",
                           "connect <env-name>",
...
syncLinkHelpMsg = HelpMsg("sync link", "Synchronizes the link between the endpoints",
                          "sync link <link-name>", ["sync link faber"])

pingTargetHelpMsg = HelpMsg("ping", "Pings given target's endpoint",
                          "ping <target>", ["ping faber"])

showClaimHelpMsg = HelpMsg("show claim", "Shows given claim information",
                           "show claim <claim-name>",
                           ["show claim Transcript"])

reqClaimHelpMsg = HelpMsg("request claim", "Request given claim",
                           "request claim <claim-name>",
                           ["request claim Transcript"])

showClaimReqHelpMsg = HelpMsg("show claim request", "Shows given claim request",
                           "show claim request <claim-req-name>",
                           ["show claim request Job-Application"])

acceptLinkHelpMsg = HelpMsg("accept invitation", "Accept invitation from given target",
                            "accept invitation from <target>", ["accept invitation from Faber"])

setAttrHelpMsg = HelpMsg("set attributes", "Sets given value to given attribute name",
                            "set <attr-name> to <attr-value>", ["set first_name to Alice"])

sendClaimHelpMsg = HelpMsg("send claim", "Sends given claim to given target",
                           "send claim <claim-name> to <target>", ["send claim Job-Application to Acme Corp"])